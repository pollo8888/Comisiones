                                ...
                                <div class="table-responsive">
                                    <table id="add-row" class="display table table-striped table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            $db = $database->open();
                                            try {
                                                $sql = "SELECT * FROM simf";
                                                foreach ($db->query($sql) as $row) {
                                                    $fFinal = $row['F_final'];
                                                    $today = date('Y-m-d');
                                                    $expirationDate = date('Y-m-d', strtotime('-1 day', strtotime($fFinal)));
                                                    $daysRemaining = (strtotime($expirationDate) - strtotime($today)) / (60 * 60 * 24); // Calcula los días restantes
                                                    if (strtotime($today) >= strtotime($expirationDate)) {
                                            ?>
                                                    <tr>
                                                        <td style="color:red; "><?php echo "El registro expira el dia:  "." ".$fFinal ?></td>
