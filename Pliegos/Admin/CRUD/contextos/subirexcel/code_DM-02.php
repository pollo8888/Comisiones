...
if (isset($_POST['save_excel_data'])) {
    ...
    if ($cadena2 == "CP04D") {
        ...
        if (in_array($file_ext, $allowed_ext)) {
            ...
            $data = $spreadsheet->getSheetByName('01Confirmacion_DM_HTA')->toArray();
            $count = "0";
            $Unidadd=[];
            $NumeradorG = [];
            $DenominadorF = [];
            ...
            foreach ($data as $row) {
                if ($count > 0) {
                    ...
                    $DenominadorF[] = str_replace(",", "", $row['5']);
                    $NumeradorG[] = str_replace(",", "", $row['6']);
        ...
        <body>
            ...
            <script type="text/javascript">
                Swal.fire({
                    ...
                    icon: 'error',
                    title: 'Oops...',
                    html: '<?php echo "El archivo CP04D es incorrecto o contiene otro nombre favor de corroborar! <br><br> El nombre del archivo introducido es: <br>" . $fileName ?> ',
...
if ($cadena2 == "CP04D") {
    ...
    $nombreDelDocumento = "Estadísticas DM02.xlsx";
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment;filename="' . $nombreDelDocumento . '"');
    header('Cache-Control: max-age=0');

    $spreadsheet = PhpOffice\PhpSpreadsheet\IOFactory::load('plantillaDM02.xlsx');
    $worksheet = $spreadsheet->SetActiveSheetIndex(0);
    $worksheet->setTitle("Estadísticas DM02");
